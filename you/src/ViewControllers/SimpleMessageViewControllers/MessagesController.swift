...
//        observeMessages()
    ...
    fileprivate func fetchMessageWithMessageId(_ messageId: String) {
        ...
        messagesReference.observeSingleEvent(of: .childAdded, with: { (snapshot) in
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let ref = Database.database().reference().child("users").child("friends").child(chatPartnerId)
            ...
            if let value = snapshot.value {
                ...
                _ = try? Static.youStack.perform(
                    synchronous: { (transaction) in
                        ...
                        DispatchQueue.main.async(execute: {
                            self.showChatControllerForUser(friend, navigationController: self.navigationController)
                        })
                })
            }
...
//        titleView.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(showChatController)))
    ...
    func showChatControllerForUser(_ friend: Friend, navigationController: UINavigationController?) {
